...
parser.add_argument('--save_inter_param', type=bool, default=False, help='是否存储中间参数')
parser.add_argument('--ready_to_unlearn', type=bool, default=False, help='是单纯训练看一下结果吗')
parser.add_argument('--gamma', type=float, default=0.9, help='gamma of schedule')
parser.add_argument('--step_size', type=int, default=4, help='step_size of schedule')
...
step_size_list = [0.95,0.9,0.85,0.8]
gamma_list = [5,10,15,20,25,30]
#0.95 2可以的
for my_step_size in gamma_list:
    for my_gamma in step_size_list:
        ...
        scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=my_step_size, gamma=my_gamma)
        # Train the model
        epochs = args.epochs
        ...
        for epoch in range(epochs):
            ...
            if args.save_inter_param == True:
                torch.save (model.state_dict (), './intermediate_parameters/model1_'+str(trainset)+'_.pt')
                torch.save(model_param_list,'./intermediate_parameters/model_param_list_'+str(trainset)+'_.pt')
                torch.save (scheduler, './intermediate_parameters/scheduler1_'+str(trainset)+'_.pt')
                torch.save (nable_grad_u_list, './intermediate_parameters/nable_grad_u_list1_'+str(trainset)+'_.pt')
                torch.save (hvp_nable_grad_list, './intermediate_parameters/hvp_nable_grad_list1_'+str(trainset)+'_.pt')
                torch.save (train_data_ids, './intermediate_parameters/train_data_ids1_'+str(trainset)+'_.pt')
